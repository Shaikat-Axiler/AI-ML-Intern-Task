...
class ModelManager:
    ...
    async def generate(self, model, prompt, technique, system_prompt, temperature, max_tokens) -> dict:
        ...
        try:
            if cfg["provider"] == "openrouter":
                response = await self._call_openrouter(cfg, system_msg, user_msg, temperature, max_tokens)
            elif cfg["provider"] == "ollama":
                response = await self._call_ollama(cfg, system_msg, user_msg, temperature, max_tokens)
            else:
    ...
    async def _call_openrouter(self, cfg, system_msg, user_msg, temperature, max_tokens) -> dict:
        api_key = os.getenv("OPENROUTER_API_KEY", "")
        if not api_key:
    ...
    async def _call_ollama(self, cfg, system_msg, user_msg, temperature, max_tokens) -> dict:
